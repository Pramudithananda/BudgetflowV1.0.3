...
import { getExpenses, listenExpenses, getEvents } from '../../services/asyncStorageService';
...
export default function AllExpensesScreen() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [expensesDataRaw, eventsData] = await Promise.all([
        getExpenses(),
        getEvents()
      ]);
      ...
      setFilteredExpenses(expensesData.filter(e => !e.isHidden));
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenExpenses(null, (expensesLive) => {
      ...
      setFilteredExpenses(prev => {
        const visibleExpenses = expensesData.filter(e => !e.isHidden);
        if (statusFilter === 'all') return visibleExpenses;
        return visibleExpenses.filter(e => e.status === statusFilter);
      });
  ...
  useEffect(() => {
    ...
    if (statusFilter === 'all') {
      ...
      setFilteredExpenses(visibleExpenses.filter(expense => expense.status === statusFilter));
    }
