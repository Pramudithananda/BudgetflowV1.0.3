...
import { getFunders, addFunder, updateFunder, deleteFunder, listenFunders } from '../../services/asyncStorageService';
...
export default function FundersScreen() {
  ...
  useEffect(() => {
    fetchFunders();
    // Real-time funders updates
    const unsubscribe = listenFunders((fundersLive) => {
      setFunders(fundersLive);
    });
  ...
  return (
    ...
    >
      <Card style={styles.headerCard}>
        <Text style={styles.title}>{t('funders')}</Text>
        <Button
          title={showAddForm ? t('cancel') : `+ ${t('addNewFunder')}`}
      ...
      {showAddForm && (
        <Card style={styles.formCard}>
          <Text style={styles.label}>{t('funderName')}</Text>
          <TextInput
            style={[styles.input, { 
      ...
      {funders.length === 0 ? (
        <Card style={styles.emptyCard}>
          <RNView style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>{t('noFundersYet')}</Text>
            <Text style={styles.emptySubtext}>
              {t('addFundersToExpenses')}
            </Text>
          </RNView>
        </Card>
      ) : (
